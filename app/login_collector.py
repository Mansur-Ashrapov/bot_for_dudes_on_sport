...
class LoginCollector:
    def __init__(self, new_users_repo: NewUserRepository, users_repo: UserRepository, posts_repo: PostsRepository, history_repo: HistoryRepository) -> None:
    ...
    async def collect_users(self, channels, client):
        # channels = await self.channels_repo.get_all()
        await self._update_group_participants(client)
        for channel in channels:
            messages = await client.get_messages(channel, limit=10)
            for message in messages:
                message_db = await self.posts_repo.get(PostDataclass(channel, message.id, 0))
                
                last_id_comm = await self._get_last_comm_id(client, channel, message.id)
                last_id_comm_db = message_db.last_id_comm if message_db is not None else 0
                if last_id_comm > last_id_comm_db:
                    try:
                        count_new_users_post = await self._collect_users_from_post(client, channel, message.id, last_id_comm_db)
    ...
    async def _collect_users_from_post(self, client, channel, message_id, last_id):
        count_new_users_post = 0
        async for comm in client.iter_messages(channel, reply_to=message_id, offset_id=last_id):
            try:
            ...
            if user_in_current_users is None and user_in_new_users is None:
                username = await self._get_username(client, sender_id)
    ...
    async def _get_last_comm_id(self, client, channel, id):
        id = None
        async for comm in client.iter_messages(channel, reply_to=id, limit=1):
            return comm.id
    

    async def _get_username(self, client, user_id):
        user = await client.get_entity(user_id)
        return user.username
    

    async def _update_group_participants(self, client):
        participants = await client.get_participants('https://t.me/on_sport_dudes')
